...
//include("../__lib.includes/config.inc.php");
    ...
    mysqli_options($g_link, MYSQLI_OPT_LOCAL_INFILE, true);
    ...
    set_time_limit(3000); 
    $emails = imap_search($inbox,'UNSEEN SUBJECT "WBR2. Investor Transactions for a Period" SINCE "31 Jan 2018" FROM "donotreply@camsonline.com"');
        ...
        foreach ($last_cams_ids as $email_number) 
        {
            ...
            if(file_exists($file))
            {
                ...
                if($file_extension == "dbf") 
                {
                    echo "HI";
                        //Download Zip file
                     	// Password of Cams : mantra12
                        $command = "7za e \"".$file."\" -pmantra12 -aoa -o\"".$importCAMPath."\"";
                        echo "<br><br><br><br>cmd:".$command."<br>";
                        ob_start();
                        ...
                        // {
                            ...
                            $convert = 'dbfdump.pl --fs "," '.$file_name_path.' > '.$path.$csvname;
                            //echo "<br>convert:-".$convert."<br>";
                            ob_start();
                            $get_csv = shell_exec($convert); 
                            ob_end_clean(); 
                            //echo "<br><br><br><br>DBF2CSV:-<br><br>".$convert."";
                            //echo "<br>Converstion:-".$get_csv;
                            echo "<br><br><br>FILE EXIST<br><br><br>";
                        }
                        else
                        {
                        ...
                        if ($run) 
                        {
                            ...
                            include_once("/var/www/html/__lib.includes/config.inc.php");
                            //echo "HI";
                            $mutualFund->updateCAMData();
                        }
                        else
                        {
                            echo "<br>FILE NOT UPDATED INTO DATABASE";
