...
if (!is_dir($CONFIG->customerFilePath))
{
	@mkdir($CONFIG->customerFilePath);
	@chmod($CONFIG->customerFilePath,0777);
}

if (!is_dir($CONFIG->customerProfileImg))
{
	@mkdir($CONFIG->customerProfileImg);
	@chmod($CONFIG->customerProfileImg,0777);
}
 ...
 global $CONFIG,$customerProfile;
	 $result = array();
 	 if(!preg_match('/^image\//' , $file['type'])
		//if file type is not an image
		|| !preg_match('/\.(jpe?g|gif|png)$/' , strtolower($file['name']))
		...
		)
	 {
		...
		//save file inside current directory using a safer version of its name
	 	$save_path1 = preg_replace('/[^\w\.\- ]/', '', $file['name']);
		//$CONFIG->customerProfileImg
		$save_path = $CONFIG->customerProfileImg.preg_replace('/\.(.+)$/' , '', $save_path1).$CONFIG->loggedUserId.'.jpg';
		//thumbnail name is like filename-thumb.jpg
		$thumb_path = $CONFIG->customerProfileImg.preg_replace('/\.(.+)$/' , '', $save_path1).$CONFIG->loggedUserId.'-thumb.jpg';
		$thumb_url = preg_replace('/\.(.+)$/' , '', $save_path1).$CONFIG->loggedUserId.'-thumb.jpg';
		
		$customerProfile->setProfileImage($CONFIG->loggedUserId,$thumb_url);
		  ...
		  )
		{
			...
			//everything seems OK
			$result['status'] = 'OK';
			$result['message'] = 'Profile Photo changed successfully!';
			//include new thumbnails `url` in our result and send to browser
			$result['url'] = $CONFIG->customerProfileImgURL. $thumb_url;
			//'http://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['SCRIPT_NAME']).'/'.$thumb_path;
		}
	 }
