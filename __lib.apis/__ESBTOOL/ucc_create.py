...
def create_ucc_code(bse_user):
    ...
    #pass_dict = soap_get_password_upload(client)
    user_response = create_ucc(client, bse_user, pass_dict)

    return user_response

## fire SOAP query to create a new user on bsestar
def create_ucc(client, user_param, pass_dict):
    ...
    if (status == '100'):
        ...
        raise Exception(
            "BSE error 644: UCC creation error: %s" % response[1]
        )



vasr = create_ucc_code(bse_user)
print(vasr)
