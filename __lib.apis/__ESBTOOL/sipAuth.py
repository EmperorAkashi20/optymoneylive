...
def create_p_link(bse_user):
    ...
    #pass_dict = soap_get_password_upload(client)
    user_response = get_sip_auth_link(client, bse_user, pass_dict)

    return user_response

## fire SOAP query to create a new user on bsestar
def get_sip_auth_link(client, user_param, pass_dict):
    ...
    if (status == '100'):
        ...
        raise Exception(
            "BSE error 644: SIP auth Link not avialable: %s" % response[1]
        )
